...
static void create_log_file(fstream &logFile)
{
	...
	get_last_log(false, "SymStudio/logs", lastLogFile);

	currentLogFile = GenerateTimeDateFilename("txt");
	dst << "SymStudio/logs/" << currentLogFile.c_str();
	...
	if (logFile.is_open()) {
		delete_oldest_file(false, "SymStudio/logs");
...
#define LITERAL_SIZE(x) x, (sizeof(x) - 1)
	ostringstream dst;
	dst.write(LITERAL_SIZE("SymStudio/profiler_data/"));
...
static int run_program(fstream &logFile, int argc, char *argv[])
{
	...
	try {
		...
		program.AppInit();
		delete_oldest_file(false, "SymStudio/profiler_data");
...
static void main_crash_handler(const char *format, va_list args, void * /* param */)
{
	...
	string crashFilePath = "SymStudio/crashes";
