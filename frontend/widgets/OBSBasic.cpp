...
static void AddExtraModulePaths()
{
...
#if defined(_WIN32)
	int ret = GetProgramDataPath(base_module_dir, sizeof(base_module_dir), "SymStudio/plugins/%module%");
#elif defined(__APPLE__)
	int ret = GetAppConfigPath(base_module_dir, sizeof(base_module_dir), "SymStudio/plugins/%module%.plugin");
#else
	int ret = GetAppConfigPath(base_module_dir, sizeof(base_module_dir), "SymStudio/plugins/%module%");
#endif
	...
	GetProgramDataPath(system_legacy_module_dir, sizeof(system_legacy_module_dir), "SymStudio/plugins/%module%");
	...
	GetAppConfigPath(user_legacy_module_dir, sizeof(user_legacy_module_dir), "SymStudio/plugins/%module%");
...
OBSBasic::OBSBasic(QWidget *parent) : OBSMainWindow(parent), undo_s(ui), ui(new Ui::OBSBasic)
{
	...
	alertsDock->setObjectName(QStringLiteral("symStudioAlertsDock"));
	alertsDock->setFeatures(QDockWidget::DockWidgetClosable | QDockWidget::DockWidgetMovable |
				QDockWidget::DockWidgetFloatable);
	alertsDock->setWindowTitle(QStringLiteral("Alerts"));
